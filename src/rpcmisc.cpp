...
Value getinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"nitrosend_balance\": xxxxxx, (numeric) the anonymized aspire balance of the wallet\n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in aspire/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in aspire/kb\n"
    ...
    if (pwalletMain) {
        ...
        if(!fLiteMode)
            obj.push_back(Pair("nitrosend_balance",       ValueFromAmount(pwalletMain->GetAnonymizedBalance())));
    }
#endif
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"aspireaddress\"\n"
            "\nReturn information about the given aspire address.\n"
            "\nArguments:\n"
            "1. \"aspireaddress\"     (string, required) The aspire address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"aspireaddress\", (string) The aspire address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Aspire address and we have full public key:
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are aspire addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) aspire address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"aspireaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"aspireaddress\"  (string, required) The aspire address to use for the signature.\n"
