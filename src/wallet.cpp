...
// Recursively determine the rounds of a given input (How deep is the Nitrosend chain for a given input)
int CWallet::GetRealInputNitrosendRounds(CTxIn in, int rounds) const
{
    ...
    if(wtx != NULL)
    {
        ...
        {
            LogPrint("nitrosend", "GetInputNitrosendRounds INSERTING %s\n", hash.ToString());
        ...
        if(nout >= wtx->vout.size())
        {
            ...
            LogPrint("nitrosend", "GetInputNitrosendRounds UPDATED   %s %3d %3d\n", hash.ToString(), nout, -4);
            return -4;
        }
        ...
        BOOST_FOREACH(CTxIn in2, wtx->vin)
        {
            if(IsMine(in2))
            {
                int n = GetRealInputNitrosendRounds(in2, rounds+1);
...
int CWallet::GetInputNitrosendRounds(CTxIn in) const {
    LOCK(cs_wallet);
    int realNitrosendRounds = GetRealInputNitrosendRounds(in, 0);
    return realNitrosendRounds > nNitrosendRounds ? nNitrosendRounds : realNitrosendRounds;
}
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                int rounds = GetInputNitrosendRounds(vin);
                fTotal += (float)rounds;
                fCount += 1;
            }
        }
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                int rounds = GetInputNitrosendRounds(vin);
                nTotal += pcoin->vout[i].nValue * rounds / nNitrosendRounds;
            }
        }
    }
...
bool less_then_denom (const COutput& out1, const COutput& out2)
{
    ...
    bool found1 = false;
    bool found2 = false;
    BOOST_FOREACH(int64_t d, nitroSendDenominations) // loop through predefined denoms
    {
...
bool CWallet::SelectCoins(const CAmount& nTargetValue, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX) const
{
    ...
    if (coinControl && coinControl->HasSelected())
    {
        BOOST_FOREACH(const COutput& out, vCoins)
        {
            ...
            if(coin_type == ONLY_DENOMINATED) {
                ...
                if(rounds < nNitrosendRounds) continue;
            }
    ...
    if(coin_type == ONLY_DENOMINATED) {
        ...
        BOOST_FOREACH(int64_t v, nitroSendDenominations)
        {
            BOOST_FOREACH(const COutput& out, vCoins)
            {
...
bool CWallet::SelectCoinsByDenominations(int nDenom, int64_t nValueMin, int64_t nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, int64_t& nValueRet, int nNitrosendRoundsMin, int nNitrosendRoundsMax)
{
    vCoinsRet.clear();
    nValueRet = 0;
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(nValueRet + out.tx->vout[out.i].nValue <= nValueMax){
            ...
            int rounds = GetInputNitrosendRounds(vin);
            if(rounds >= nNitrosendRoundsMax) continue;
            if(rounds < nNitrosendRoundsMin) continue;
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& setCoinsRet, CAmount& nValueRet, int nNitrosendRoundsMin, int nNitrosendRoundsMax) const
{
    ...
    vector<COutput> vCoins;
    AvailableCoins(vCoins, true, coinControl, nNitrosendRoundsMin < 0 ? ONLY_NONDENOMINATED_NOT1000IFMN : ONLY_DENOMINATED);
...
{
    return  nInputAmount != 0 && nInputAmount % NITROSEND_COLLATERAL == 0 && nInputAmount < NITROSEND_COLLATERAL * 5 && nInputAmount > NITROSEND_COLLATERAL;
}
...
bool CWallet::CreateCollateralTransaction(CMutableTransaction& txCollateral, std::string& strReason)
{
    ...
    */
    ...
    {
        strReason = "Error: Nitrosend requires a collateral transaction and could not locate an acceptable input!";
        return false;
    }
    ...
    if(nValueIn2 - NITROSEND_COLLATERAL - nFeeRet > 0) {
        //pay collateral charge in fees
        CTxOut vout3 = CTxOut(nValueIn2 - NITROSEND_COLLATERAL, scriptChange);
    ...
    BOOST_FOREACH(CTxIn v, txCollateral.vin) {
        if(!SignSignature(*this, v.prevPubKey, txCollateral, vinNumber, int(SIGHASH_ALL|SIGHASH_ANYONECANPAY))) {
            ...
            strReason = "CNitrosendPool::Sign - Unable to sign collateral transaction! \n";
            return false;
        }
        vinNumber++;
    }
            ...
            while (true)
            {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX))
                {
                    if(coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 1000 ASPIRE.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT1000IFMN) {
                        strFailReason = _("Unable to locate enough Nitrosend non-denominated funds for this transaction that are not equal 1000 ASPIRE.");
                    } else {
                        strFailReason = _("Unable to locate enough Nitrosend denominated funds for this transaction.");
                        strFailReason += " " + _("Nitrosend uses exact denominated amounts to send funds, you might simply need to anonymize some more coins.");
                    }
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-aspire-address
                    CScript scriptChange;
...
string CWallet::PrepareNitrosendDenominate(int minRounds, int maxRounds)
{
    if (IsLocked())
        return _("Error: Wallet locked, unable to create transaction!");

    if(nitroSendPool.GetState() != POOL_STATUS_ERROR && nitroSendPool.GetState() != POOL_STATUS_SUCCESS)
        if(nitroSendPool.GetEntriesCount() > 0)
            return _("Error: You already have pending entries in the Nitrosend pool");
    ...
    */
    if(minRounds >= 0){
        if (!SelectCoinsByDenominations(nitroSendPool.sessionDenom, 0.1*COIN, NITROSEND_POOL_MAX, vCoins, vCoins2, nValueIn, minRounds, maxRounds))
            return _("Error: Can't select current denominated inputs");
    }

    LogPrintf("PrepareNitrosendDenominate - preparing nitrosend denominate . Got: %d \n", nValueIn);
    ...
    */
    ...
    while(nStep < nStepsMax) {
        ...
        BOOST_FOREACH(int64_t v, nitroSendDenominations){
            ...
            bool fAccepted = false;
            if((nitroSendPool.sessionDenom & (1 << 0))      && v == ((100*COIN) +100000)) {fAccepted = true;}
            else if((nitroSendPool.sessionDenom & (1 << 1)) && v == ((10*COIN)  +10000)) {fAccepted = true;}
            else if((nitroSendPool.sessionDenom & (1 << 2)) && v == ((1*COIN)   +1000)) {fAccepted = true;}
            else if((nitroSendPool.sessionDenom & (1 << 3)) && v == ((.1*COIN)  +100)) {fAccepted = true;}
            if(!fAccepted) continue;
    ...
    if(nitroSendPool.GetDenominations(vOut) != nitroSendPool.sessionDenom) {
    ...
    nitroSendPool.SendNitrosendDenominate(vCoinsResult, vOut, nValueIn - nValueLeft);
