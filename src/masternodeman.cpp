...
void CMasternodeMan::ProcessMasternodeConnections()
{
    ...
    BOOST_FOREACH(CNode* pnode, vNodes) {
        if(pnode->fNitroSendMaster){
            if(nitroSendPool.pSubmittedToMasternode != NULL && pnode->addr == nitroSendPool.pSubmittedToMasternode->addr) continue;
...
void CMasternodeMan::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "mnb") { //Masternode Broadcast
        ...
        if(!nitroSendSigner.IsVinAssociatedWithPubkey(mnb.vin, mnb.pubkey)) {
        ...
        //  - this is checked later by .check() in many places and by ThreadCheckNitroSendPool()
    ...
    // Light version for OLD MASSTERNODES - fake pings, no self-activation
    else if (strCommand == "dsee") { //NitroSend Election Entry
        ...
        if(!nitroSendSigner.VerifyMessage(pubkey, vchSig, strMessage, errorMessage)){
        ...
        if(Params().NetworkID() == CBaseChainParams::MAIN){
            if(addr.GetPort() != 6180) return;
        } else if(addr.GetPort() == 6180) return;
        ...
        // make sure the vout that was signed is related to the transaction that spawned the Masternode
        //  - this is expensive, so it's only done once per Masternode
        if(!nitroSendSigner.IsVinAssociatedWithPubkey(vin, pubkey)) {
        ...
        CTxOut vout = CTxOut(199999.99*COIN, nitroSendPool.collateralPubKey);
        ...
        if(fAcceptable){
            ...
            // should be at least not earlier than block when 1000 ASPIRE tx got MASTERNODE_MIN_CONFIRMATIONS
            uint256 hashBlock = 0;
            CTransaction tx2;
            ...
            {
                CBlockIndex* pMNIndex = (*mi).second; // block for 1000 ASPIRE tx -> 1 confirmation
    ...
    else if (strCommand == "dseep") { //NitroSend Election Entry Ping
