...
bool CMasternodeBroadcast::CheckAndUpdate(int& nDos)
{
    ...
    if(!nitroSendSigner.VerifyMessage(pubkey, sig, strMessage, errorMessage)){
    ...
    if(Params().NetworkID() == CBaseChainParams::MAIN) {
        if(addr.GetPort() != 6180) return false;
    } else if(addr.GetPort() == 6180) return false;
...
bool CMasternodeBroadcast::Sign(CKey& keyCollateralAddress)
{
    ...
    if(!nitroSendSigner.SignMessage(strMessage, errorMessage, sig, keyCollateralAddress)) {
...
bool CMasternodePing::Sign(CKey& keyMasternode, CPubKey& pubKeyMasternode)
{
    ...
    if(!nitroSendSigner.SignMessage(strMessage, errorMessage, vchSig, keyMasternode)) {
        LogPrintf("CMasternodePing::Sign() - Error: %s\n", errorMessage);
        return false;
    }

    if(!nitroSendSigner.VerifyMessage(pubKeyMasternode, vchSig, strMessage, errorMessage)) {
        LogPrintf("CMasternodePing::Sign() - Error: %s\n", errorMessage);
        return false;
    }
