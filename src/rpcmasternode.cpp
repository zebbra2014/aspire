...
Value nitrosend(const Array& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "nitrosend <aspireaddress> <amount>\n"
            "aspireaddress, reset, or auto (AutoDenominate)"
    ...
    if(params[0].get_str() == "auto"){
        if(fMasterNode)
            return "NitroSend is not supported from masternodes";

        return "DoAutomaticDenominating " + (nitroSendPool.DoAutomaticDenominating() ? "successful" : ("failed: " + nitroSendPool.GetStatus()));
    }

    if(params[0].get_str() == "reset"){
        nitroSendPool.Reset();
        return "successfully reset nitrosend";
    }

    if (params.size() != 2)
        throw runtime_error(
            "nitrosend <aspireaddress> <amount>\n"
            "aspireaddress, denominate, or auto (AutoDenominate)"
            "<amount> is a real and will be rounded to the next 0.1"
            + HelpRequiringPassphrase());
...
Value getpoolinfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("state",        nitroSendPool.GetState()));
    obj.push_back(Pair("entries",      nitroSendPool.GetEntriesCount()));
    obj.push_back(Pair("entries_accepted",      nitroSendPool.GetCountEntriesAccepted()));
    return obj;
}
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp  ||
        ...
        throw runtime_error(
                ...
                "  start        - Start masternode configured in aspire.conf\n"
