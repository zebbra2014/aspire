...
#include "config/aspire-config.h"
#endif
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += "  -conf=<file>           " + strprintf(_("Specify configuration file (default: %s)"), "aspire.conf") + "\n";
...
#ifndef WIN32
    strUsage += "  -pid=<file>            " + strprintf(_("Specify pid file (default: %s)"), "aspired.pid") + "\n";
#endif
    ...
    strUsage += "  -port=<port>           " + strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 6180, 16180) + "\n";
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += "  -mintxfee=<amt>          " + strprintf(_("Fees (in ASPIRE/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"), FormatMoney(CWallet::minTxFee.GetFeePerK())) + "\n";
    strUsage += "  -paytxfee=<amt>          " + strprintf(_("Fee (in ASPIRE/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())) + "\n";
    ...
    strUsage += "                           aspire (or specifically: nitrosend, instantx, masternode, keepass, mnpayments, mnbudget)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        strUsage += ", qt";
    strUsage += ".\n";
#ifdef ENABLE_WALLET
    ...
    strUsage += "  -minrelaytxfee=<amt>   " + strprintf(_("Fees (in ASPIRE/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())) + "\n";
    ...
    strUsage += "  -litemode=<n>          " + strprintf(_("Disable all Aspire specific functionality (Masternodes, Nitrosend, InstantX, Budgeting) (0-1, default: %u)"), 0) + "\n";
    ...
    strUsage += "  -masternodeaddr=<n>        " + strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:6180") + "\n";
    ...
    strUsage += "\n" + _("Nitrosend options:") + "\n";
    strUsage += "  -enablenitrosend=<n>          " + strprintf(_("Enable use of automated nitrosend for funds stored in this wallet (0-1, default: %u)"), 0) + "\n";
    strUsage += "  -nitrosendrounds=<n>          " + strprintf(_("Use N separate masternodes to anonymize funds  (2-8, default: %u)"), 2) + "\n";
    strUsage += "  -anonymizeaspireamount=<n>     " + strprintf(_("Keep N ASPIRE anonymized (default: %u)"), 0) + "\n";
    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Nitrosend by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 6179, 16179) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2014-%i The Aspire Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that Aspire is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Aspire Core is shutting down."));
...
#endif
    // Make sure only a single Aspire process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Aspire Core is probably already running."), strDataDir));
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Aspire Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Aspire Core to complete") << "\n";
    ...
    if(fMasterNode) {
        LogPrintf("IS NITROSEND MASTER NODE\n");
        ...
        if(!strMasterNodePrivKey.empty()){
            ...
            if(!nitroSendSigner.SetKey(strMasterNodePrivKey, errorMessage, key, pubkey))
            {
    ...
    fEnableNitrosend = GetBoolArg("-enablenitrosend", false);

    nNitrosendRounds = GetArg("-nitrosendrounds", 2);
    if(nNitrosendRounds > 16) nNitrosendRounds = 16;
    if(nNitrosendRounds < 1) nNitrosendRounds = 1;

    nLiquidityProvider = GetArg("-liquidityprovider", 0); //0-100
    if(nLiquidityProvider != 0) {
        nitroSendPool.SetMinBlockSpacing(std::min(nLiquidityProvider,100)*15);
        fEnableNitrosend = true;
        nNitrosendRounds = 99999;
    }

    nAnonymizeDarkcoinAmount = GetArg("-anonymizeaspireamount", 0);
    ...
    //lite mode disables all Masternode and Nitrosend related functionality
    ...
    LogPrintf("Nitrosend rounds %d\n", nNitrosendRounds);
    LogPrintf("Anonymize Aspire Amount %d\n", nAnonymizeDarkcoinAmount);
    LogPrintf("Budget Mode %s\n", strBudgetMode.c_str());

    /* Denominations

       A note about convertability. Within Nitrosend pools, each denomination
    ...
    */
    nitroSendDenominations.push_back( (100      * COIN)+100000 );
    nitroSendDenominations.push_back( (10       * COIN)+10000 );
    nitroSendDenominations.push_back( (1        * COIN)+1000 );
    nitroSendDenominations.push_back( (.1       * COIN)+100 );
    /* Disabled till we need them
    nitroSendDenominations.push_back( (.01      * COIN)+10 );
    nitroSendDenominations.push_back( (.001     * COIN)+1 );
    */

    nitroSendPool.InitCollateralAddress();

    threadGroup.create_thread(boost::bind(&ThreadCheckNitroSendPool));
