...
#if defined(NDEBUG)
# error "Aspire cannot be compiled without assertions."
#endif
                        ...
                        bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
...

    {
        ...
        if(mapNitrosendBroadcastTxes.count(hash)) {
...
bool static AlreadyHave(const CInv& inv)
{
    ...
    case MSG_DSTX:
        return mapNitrosendBroadcastTxes.count(inv.hash);
    case MSG_BLOCK:
        ...
        const CInv &inv = *it;
        {
            ...
            else if (inv.IsKnownType())
            {
                ...
                if (!pushed && inv.type == MSG_DSTX) {       
                    if(mapNitrosendBroadcastTxes.count(inv.hash)){
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss <<
                            mapNitrosendBroadcastTxes[inv.hash].tx <<
                            mapNitrosendBroadcastTxes[inv.hash].vin <<
                            mapNitrosendBroadcastTxes[inv.hash].vchSig <<
                            mapNitrosendBroadcastTxes[inv.hash].sigTime;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    else if (strCommand == "tx"|| strCommand == "dstx")
    {
        ...
        if(strCommand == "tx") {
            ...
            if(pmn != NULL)
            {
                ...
                if(!mapNitrosendBroadcastTxes.count(tx.GetHash())){
                    ...
                    mapNitrosendBroadcastTxes.insert(make_pair(tx.GetHash(), dstx));
                }
            }
        }
    ...
    else
    {
        ...
        nitroSendPool.ProcessMessageNitrosend(pfrom, strCommand, vRecv);
