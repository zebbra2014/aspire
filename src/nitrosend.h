...
class CTxIn;
class CNitrosendPool;
class CNitroSendSigner;
class CMasterNodeVote;
class CBitcoinAddress;
class CNitrosendQueue;
class CNitrosendBroadcastTx;
...
#define NITROSEND_QUEUE_TIMEOUT                 30
#define NITROSEND_SIGNING_TIMEOUT               15

// used for anonymous relaying of inputs/outputs/sigs
#define NITROSEND_RELAY_IN                 1
#define NITROSEND_RELAY_OUT                2
#define NITROSEND_RELAY_SIG                3

static const int64_t NITROSEND_COLLATERAL = (0.01*COIN);
static const int64_t NITROSEND_POOL_MAX = (199999.99*COIN);

extern CNitrosendPool nitroSendPool;
extern CNitroSendSigner nitroSendSigner;
extern std::vector<CNitrosendQueue> vecNitrosendQueue;
extern std::string strMasterNodePrivKey;
extern map<uint256, CNitrosendBroadcastTx> mapNitrosendBroadcastTxes;
extern CActiveMasternode activeMasternode;

/** Holds an Nitrosend input
 */
...
// A clients transaction in the nitrosend pool
class CNitroSendEntry
{
public:
    bool isSet;
    ...
    /// Add entries to use for Nitrosend
    ...
    bool IsExpired()
    {
        return (GetTime() - addedTime) > NITROSEND_QUEUE_TIMEOUT;// 120 seconds
    }
};


/**
 * A currently inprogress Nitrosend merge and denomination information
 */
class CNitrosendQueue
{
public:
    ...
    /** Sign this Nitrosend transaction
    ...
    /// Is this Nitrosend expired?
    bool IsExpired()
    {
        return (GetTime() - time) > NITROSEND_QUEUE_TIMEOUT;// 120 seconds
    }
...
/** Helper class to store Nitrosend transaction (tx) information.
 */
class CNitrosendBroadcastTx
{
public:
...
{
public:
    /// Is the inputs associated with this public key? (and there is 1000 ASPIRE - checking if valid masternode)
...
/** Used to keep track of current status of Nitrosend pool
 */
class CNitrosendPool
{
private:
    mutable CCriticalSection cs_nitrosend;

    std::vector<CNitroSendEntry> entries; // Masternode/clients entries
...
public:
    ...
    CNitrosendPool()
    {
        /* Nitrosend uses collateral addresses to trust parties entering the pool
    ...
    /** Process a Nitrosend message using the Nitrosend protocol
     * \param pfrom
     * \param strCommand lower case command string; valid values are:
     *        Command  | Description
     *        -------- | -----------------
     *        dsa      | Nitrosend Acceptable
     *        dsc      | Nitrosend Complete
     *        dsf      | Nitrosend Final tx
     *        dsi      | Nitrosend vIn
     *        dsq      | Nitrosend Queue
     *        dss      | Nitrosend Signal Final Tx
     *        dssu     | Nitrosend status update
     *        dssub    | Nitrosend Subscribe To
     * \param vRecv
     */
    void ProcessMessageNitrosend(CNode* pfrom, std::string& strCommand, CDataStream& vRecv);

    void InitCollateralAddress(){
        SetCollateralAddress(Params().NitrosendPoolDummyAddress());
    }
    ...
    void UpdateState(unsigned int newState)
    {
        if (fMasterNode && (newState == POOL_STATUS_ERROR || newState == POOL_STATUS_SUCCESS)){
            LogPrint("nitrosend", "CNitrosendPool::UpdateState() - Can't set state to ERROR or SUCCESS as a Masternode. \n");
            return;
        }

        LogPrintf("CNitrosendPool::UpdateState() == %d | %d \n", state, newState);
        if(state != newState){
    ...
    /// Passively run Nitrosend in the background according to the configuration in settings (only for QT)
    bool DoAutomaticDenominating(bool fDryRun=false);
    bool PrepareNitrosendDenominate();

    /// Check for process in Nitrosend
    void Check();
    ...
    void SendNitrosendDenominate(std::vector<CTxIn>& vin, std::vector<CTxOut>& vout, int64_t amount);
    /// Get Masternode updates about the progress of Nitrosend
    ...
    /// Get the denominations for a specific amount of aspire.
