...
static bool AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-help"))
    {
        ...
        std::string strUsage = _("Aspire Core aspire-tx utility version") + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  aspire-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded aspire transaction") + "\n" +
              "  aspire-tx [options] -create [commands]   " + _("Create hex-encoded aspire transaction") + "\n" +
              "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded aspire transaction
