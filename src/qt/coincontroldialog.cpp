    ...
    model(0)
{
    ...
    ui->treeWidget->setColumnWidth(COLUMN_NITROSEND_ROUNDS, 88);
...
void CoinControlDialog::viewItemChanged(QTreeWidgetItem* item, int column)
{
    if (column == COLUMN_CHECKBOX && item->text(COLUMN_TXHASH).length() == 64) // transaction hash is 64 characters (this means its a child node, so its not a parent node in tree mode)
    {
        ...
        if (item->checkState(COLUMN_CHECKBOX) == Qt::Unchecked)
            ...
            CTxIn vin(outpt);
            int rounds = pwalletMain->GetInputNitrosendRounds(vin);
            if(coinControl->useNitroSend && rounds < nNitrosendRounds) {
                QMessageBox::warning(this, windowTitle(),
                    tr("Non-anonymized input selected. <b>Nitrosend will be disabled.</b><br><br>If you still want to use Nitrosend, please deselect all non-nonymized inputs first and then check Nitrosend checkbox again."),
                    QMessageBox::Ok, QMessageBox::Ok);
                coinControl->useNitroSend = false;
            }
        }
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    if (nQuantity > 0)
    {
        ...
        if (nPayAmount > 0)
        {
            ...
            // DS Fee = overpay
            if(coinControl->useNitroSend && nChange > 0)
            {
                nPayFee += nChange;
                nChange = 0;
            }
    ...
    int nDisplayUnit = BitcoinUnits::ASPIRE;
    ...
    BOOST_FOREACH(PAIRTYPE(QString, vector<COutput>) coins, mapCoins)
    {
        ...
        BOOST_FOREACH(const COutput& out, coins.second)
        {
            ...
            if(ExtractDestination(out.tx->vout[out.i].scriptPubKey, outputAddress))
            {
                ...
                // if listMode or change => show aspire address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            if(rounds >= 0) itemOutput->setText(COLUMN_NITROSEND_ROUNDS, strPad(QString::number(rounds), 11, " "));
            else itemOutput->setText(COLUMN_NITROSEND_ROUNDS, strPad(QString(tr("n/a")), 11, " "));
