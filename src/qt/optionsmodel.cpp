...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::ASPIRE);
    ...
    if (!settings.contains("nNitrosendRounds"))
        settings.setValue("nNitrosendRounds", 2);

    if (!settings.contains("nAnonymizeDarkcoinAmount"))
        settings.setValue("nAnonymizeDarkcoinAmount", 1000);

    nNitrosendRounds = settings.value("nNitrosendRounds").toLongLong();
    ...
    if (settings.contains("nNitrosendRounds"))
        SoftSetArg("-nitrosendrounds", settings.value("nNitrosendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeDarkcoinAmount"))
        SoftSetArg("-anonymizeaspireamount", settings.value("nAnonymizeDarkcoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in aspire.cpp during shotdown to also remove the window positions
